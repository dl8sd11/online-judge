...
int main()
{
  ...
  ans = 0x3f3f3f3f;
  for(int i=1;i<N;i++){
    if(lcpa[i-1]>=lcpa[i])continue;
    if(i+1<N&&lcpa[i+1]>=lcpa[i])continue;
    int ret = lcpa[i-1]+1;
    if(i+1<N)ret = max(ret,lcpa[i+1]+1);
    debug(ret,sa[i],sa[i-1]);
    if(min(sa[i],sa[i-1])<SZ(a)&&max(sa[i],sa[i-1])>=SZ(a)+1&&min(sa[i],sa[i-1])+ret<=SZ(a)) {
      ans = min(ans,ret);
      debug(i);
    }
  }
