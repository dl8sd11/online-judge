...
ll pt[16800000];
vector<ll> cover[26];
pdd solve(double x1,double x2,double y1,double y2){
  double delta = (x1*x1*x2-x2*x2*x1);
  double deltaA = (y1*x2-y2*x1);
  double deltaB = (x1*x1*y2-x2*x2*y1);
  if(delta==0||(deltaA/delta)>=0)return mp(87,87);
  return mp(deltaA/delta,deltaB/delta);
}
ll getans(ll status){
...
int main()
{
  ...
  while(T--){
    ...
    for(auto para:lines) {
      ll fliter = 0;
      REP(i,n){
        if(abs(pig[i].X*pig[i].X*para.X+pig[i].X*para.Y-pig[i].Y)<1e-6){
          fliter += (1<<i);
        }
      }
      debug(fliter);
