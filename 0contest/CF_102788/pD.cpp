...
void build () {
    N = SZ(len);
    seg.resize(N * 2);
    for (int i=0; i<N; i++) seg[i+N] = len[i];
    for (int i=N-1; i>0; i--) {
...
int mgcd (int x, int y) {
    return x == 0 ? y : __gcd(x, y);
}

int qgcd (int l, int r) {
    int res = 0;
    for (l+=N, r+=N; l<r; l>>=1, r>>=1) {
        if (l&1) res = mgcd(res, seg[l++]);
        if (r&1) res = mgcd(res, seg[--r]);
    }
    return res;
}
signed main () {
    ...
    for (auto [l, r] : qry) {
        ...
        while (head < l) {
            ms.erase(ms.find(len[head++]));
        }
        debug(qgcd(l, r));
        if (ms.count(qgcd(l, r))) {
            cout << "Y" << qgcd(l, r) << endl;
        } else {
            cout << "N" << endl;
        }
