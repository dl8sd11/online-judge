...
namespace LG {
    ...
    void upd(int idx,int nd) {
        auto it = seg[idx].find(w[nd]);
        if (it == seg[idx].end() || it->Y > nd) {
            seg[idx][w[nd]] = nd;
        }
    }
...
pii query(int v,int k) {
    int mx = w[v];//SEG::binary_search(k,19,SEG::root[in[v]],SEG::root[out[v]]);
...
int main()
{
    ...
    while (t--) {
        ...
        while (q--) {
            ...
            // cin >> a >> b;
            input(a);input(b);
            tie(v,x) = query(rand()%n+1,123);//query(a^v,b^x);
            printf("%d %d\n",v,x);
            // cout << v << " " << x << endl;
        }
