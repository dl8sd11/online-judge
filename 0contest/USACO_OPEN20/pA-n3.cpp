    ...
    for (int j=1; j<=n; j++) {
        for (int i=0; i<=n; i++) {
            ...
            for (int k=0; k<i; k++) {
                ll up = sum(k+1, j, i, n);
                ll dw = sum(i+1, j, n, j);
                if (i != k && op[i][j] && op[i+1][j]) {
                    dp[i][j][1] += dp[k][j-1][1] * mpow(2, up+dw-2) % MOD;
                    dp[i][j][1] %= MOD;
                }
                if (i != k && op[i][j]) {
                    dp[i][j][0] += dp[k][j-1][1] * mpow(2, up+dw-1) %MOD;
                    dp[i][j][1] %= MOD;
                }
            }
            ...
            if (op[i+1][j]) {
//                debug(i, j, col);
                dp[i][j][1] += dp[i][j-1][0] * mpow(2, col-1) % MOD;
                dp[i][j][1] %= MOD;
            }
            dp[i][j][0] += dp[i][j-1][0] * mpow(2, col) % MOD;
            dp[i][j][0] %= MOD;
