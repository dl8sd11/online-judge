#include <bits/stdc++.h>
using namespace std;
typedef long long ll;
typedef pair<ll, ll> pii;
typedef pair<double,double> pdd;
#define SQ(i) ((i)*(i))
#define MEM(a, b) memset(a, (b), sizeof(a))
#define SZ(i) int(i.size())
#define FOR(i, j, k, in) for (int i=j ; i<k ; i+=in)
#define RFOR(i, j, k, in) for (int i=j ; i>=k ; i-=in)
#define REP(i, j) FOR(i, 0, j, 1)
#define REP1(i,j) FOR(i, 1, j+1, 1)
#define RREP(i, j) RFOR(i, j, 0, 1)
#define ALL(_a) _a.begin(),_a.end()
#define mp make_pair
#define pb push_back
#define eb emplace_back
#define X first
#define Y second
#ifdef tmd
#define debug(...) do{\
    fprintf(stderr,"%s - %d (%s) = ",__PRETTY_FUNCTION__,__LINE__,#__VA_ARGS__);\
    _do(__VA_ARGS__);\
}while(0)
template<typename T>void _do(T &&_x){cerr<<_x<<endl;}
template<typename T,typename ...S> void _do(T &&_x,S &&..._t){cerr<<_x<<" ,";_do(_t...);}
template<typename _a,typename _b> ostream& operator << (ostream &_s,const pair<_a,_b> &_p){return _s<<"("<<_p.X<<","<<_p.Y<<")";}
template<typename It> ostream& _OUTC(ostream &_s,It _ita,It _itb)
{
    _s<<"{";
    for(It _it=_ita;_it!=_itb;_it++)
    {
        _s<<(_it==_ita?"":",")<<*_it;
    }
    _s<<"}";
    return _s;
}
template<typename _a> ostream &operator << (ostream &_s,vector<_a> &_c){return _OUTC(_s,ALL(_c));}
template<typename _a> ostream &operator << (ostream &_s,set<_a> &_c){return _OUTC(_s,ALL(_c));}
template<typename _a> ostream &operator << (ostream &_s,deque<_a> &_c){return _OUTC(_s,ALL(_c));}
template<typename _a,typename _b> ostream &operator << (ostream &_s,map<_a,_b> &_c){return _OUTC(_s,ALL(_c));}
template<typename _t> void pary(_t _a,_t _b){_OUTC(cerr,_a,_b);cerr<<endl;}
#define IOS()
#else
#define debug(...)
#define pary(...)
#define endl '\n'
#define IOS() ios_base::sync_with_stdio(0);cin.tie(0)
#endif

const ll MOD = 1000000007;
const ll INF = 0x3f3f3f3f3f3f3f3f;
const int iNF = 0x3f3f3f3f;
// const ll MAXN = 

const double EPS = 1e-9;
int sign(double x) {
  if (x < -EPS) return -1;
  if (x > +EPS) return +1;
  return 0;
}

struct point {
  double x, y;
  point &operator+=(point p) { x += p.x; y += p.y; return *this; }
  point &operator-=(point p) { x -= p.x; y -= p.y; return *this; }
  point &operator*=(double a)     { x *= a;   y *= a;   return *this; }
  point &operator/=(double a)     { return *this *= (1.0/a); }
  point operator-() const    { return {-x, -y}; }
  bool operator<(point p) const {
    int s = sign(x - p.x);
    return s ? s < 0 : sign(y - p.y) < 0;
  }
};
point operator-(point p, point q) { return p -= q; }
point operator*(double a, point p) { return p *= a; }
point operator+(point p, point q) { return p += q; }

struct line { point p, q; };
double cross(point p, point q) { return p.x*q.y-p.y*q.x; } // left turn > 0
point intersect(line l, line m) {
    auto a = cross(m.q - m.p, l.q - l.p);
    auto b = cross(l.p - m.p, l.q - l.p);
    if ( sign(a)) return m.p + b/a*(m.q - m.p); // properly crossing
    if (!sign(b)) return m.p, m.q;              // same line
    return {};                                    // disjoint parallel
}

bool isInt(double x) {
    if (abs(x-ceil(x)) < EPS || abs(x-floor(x)) < EPS) {
        return true;
    } else {
        return false;
    }
}

ll getInt(double x) {
    if (abs(x-ceil(x))) {
        return ceil(x);
    } else {
        return floor(x);
    }
}
int n;
vector<line> pts;
set<point> ipt;
/********** Good Luck :) **********/
int main()
{
    IOS();
    cin >> n;
    ll ans = 0;
    REP (i, n) {
        ll xf, yf, xs, ys;
        cin >> xf >> yf >> xs >> ys;
        point f = {xf, yf};
        point s = {xs, ys};
        ll d = __gcd(abs(xf-xs), abs(yf-ys));
        ans += d + 1;
        pts.push_back({f,s});
    }


    REP (i, n) {
        REP (j, i) {
            point cur = intersect(pts[i], pts[j]);
            if (isInt(cur.x) && isInt(cur.y)) {
                ans--;
                ipt.insert({getInt(cur.x), getInt(cur.y)});
            }
        }
    }
    debug(ipt.size());

    cout << ans + SZ(ipt) << endl;
    return 0;
}
