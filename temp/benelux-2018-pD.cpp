...
struct Djs {
    vector<int> djs, sz;
    Djs (int n) : djs(n), sz(n,1) {
        iota(ALL(djs), 0);
    }

    int fnd (int x) {
        return djs[x] == x ? djs[x] : djs[x] = fnd(djs[x]);
    }
...
signed main () {
    ...
    while (bfs.size()) {
        ...
        if (!dsu.mrg(cur.x, cur.y)) continue;
        bfs.push({l[cur.x], l[cur.y], cur.st+1});
        bfs.push({r[cur.x], r[cur.y], cur.st+1});
    }
